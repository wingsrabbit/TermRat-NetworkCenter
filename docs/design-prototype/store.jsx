...
function AppProvider({ children }) {
  ...
  const [auth, setAuth] = useState(() => {
    try { return JSON.parse(localStorage.getItem("termrat-auth") || "null"); } catch (e) { return null; }
  });
  const login = (user) => {
    const a = { name: user.name, role: user.role };
    setAuth(a); localStorage.setItem("termrat-auth", JSON.stringify(a));
  };
  const logout = () => { setAuth(null); localStorage.removeItem("termrat-auth"); navigate("/termadmin"); };
  const setRole = (role) => {
    setAuth((a) => { const n = { ...(a || {}), role }; localStorage.setItem("termrat-auth", JSON.stringify(n)); return n; });
  };
