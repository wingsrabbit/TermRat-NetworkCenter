...
/* ---------------- 图标（线性，16/24 viewBox 统一 stroke） ---------------- */
function Icon({ d, paths, size, fill, sw, children, style }) {
  return (
...
const ICONS = {
  ...
  rabbit: ["M13 16a3 3 0 012 5H7a3 3 0 012-5", "M8 14v.5M16 14v.5M11.25 16.25h1.5L12 17l-.75-.75z", "M4.42 11.247A13.152 13.152 0 004 14.556C4 18.728 7.582 21 12 21s8-2.272 8-6.444a11.702 11.702 0 00-.493-3.309", "M14 10c-.25-3 1-7 4-7-1 1-1.5 2.5-1.5 4M10 10c.25-3-1-7-4-7 1 1 1.5 2.5 1.5 4"],
};
function Ic({ name, size, ...rest }) {
...
/* ---------------- 基础组件 ---------------- */
function Tag({ tone, dot, children, className }) {
...
function Bar({ value, label, level }) {
  const lv = level || (window.DB ? window.DB.usageLevel(value) : "green");
  return (
...
/* —— count-up 数字 —— */
function CountUp({ value, decimals = 0, duration = 900, suffix = "", className }) {
...
/* —— KPI 卡 —— */
function StatCard({ label, icon, value, decimals, suffix, tone, sub, delay }) {
  return (
...
/* —— 确认弹窗 —— */
function Confirm({ title, message, danger, confirmText, onConfirm, onClose }) {
  return (
...
/* —— 延迟值（按阈值配色） —— */
function Latency({ ms, big }) {
  if (ms == null) return <span className="muted">—</span>;
  const lv = window.DB.latencyLevel(ms);
...
Object.assign(window, {
  Ic, Icon, Tag, StatusDot, Bar, CountUp, StatCard, Checkbox, Switch,
  Modal, Drawer, Confirm, ToastProvider, useToast, CodeBlock, Empty,
  useClickOutside, Latency,
});
